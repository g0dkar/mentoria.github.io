...
:::warning[ATENÇÃO!]
Muita coisa nas outras partes da mentoria será feita via linha de comando, e vai assumir que você sabe utilizar mesmo que de forma bem básica a linha de comando.
...
É como o computador sabe quando pedimos para ele ler um arquivo ***onde*** ele pode encontrar o tal arquivo :wink:
:::
...
<OSTabs>
  <windows>
    ![Animação mostrando navegação em pastas em um Terminal Windows PowerShell](img/20-windows-pastas-cli.gif "Animação mostrando a navegação da pasta raíz da unidade C: até a pasta vim90")
  </windows>
  <linux>
    ![Animação mostrando navegação em pastas em um Terminal Linux](img/20-linux-pastas-cli.gif "Animação mostrando a navegação da pasta do usuário atual até a pasta vim90 em um Terminal Linux")
  </linux>
</OSTabs>
...
A pasta chamada `.` representa a pasta que estamos nesse momento. Isso é útil se quisermos deixar claro que onde quer que estejamos navegando será *relativo* a onde estamos no momento. Esse tipo de navegação é chamada **Relativa** e é a forma padrão de navegar: quando você diz `cd proximo-diretorio` está subentendido que a pasta `proximo-diretorio` existe dentro da pasta que você está atualmente.

Já a pasta chamada `..` representa a pasta "acima" **da pasta que estamos atualmente**. Isso nos ajuda a "voltar" pastas que navegamos:

<OSTabs>
  <windows>
    ![Animação mostrando navegação em pastas em um Terminal Windows PowerShell](img/20-windows-pastas-cli2.gif "Animação mostrando a navegação da pasta vim90 de volta a pasta raíz da unidade C:")
  </windows>
  <linux>
    ![Animação mostrando navegação em pastas em um Terminal Linux](img/20-linux-pastas-cli2.gif "Animação mostrando a navegação da pasta do usuário atual até a pasta vim90 em um Terminal Linux")
  </linux>
</OSTabs>

Note que o `..` é sempre ***relativo*** a pasta que estamos no momento. Observe o seguinte exemplo:

<OSTabs>
  <windows>
    ![Animação mostrando navegação em pastas em um Terminal Windows PowerShell](img/20-windows-pastas-cli3.gif "Animação mostrando navegação em pastas em um Terminal Windows PowerShell, em particular demonstrando que cd .. é sempre relativo a onde estamos")
  </windows>
  <linux>
    ![Animação mostrando navegação em pastas em um Terminal Linux](img/20-linux-pastas-cli3.gif "Animação mostrando a navegação da pasta do usuário atual até a pasta vim90 em um Terminal Linux, em particular demonstrando que cd .. é sempre relativo a onde estamos")
  </linux>
</OSTabs>
...
A pasta informada não é relativa a nada, sabemos ***exatamente*** onde queremos ir.
...
<OSTabs>
  <windows>
    ![Animação mostrando como listar as pastas e arquivos do diretório atual no Windows](img/20-windows-pastas-cli4.gif "Animação mostrando como listar as pastas e arquivos do diretório atual")
  </windows>
  <linux>
    ![Animação mostrando como listar as pastas e arquivos do diretório atual no Linux](img/20-linux-pastas-cli4.gif "Animação mostrando como listar as pastas e arquivos do diretório atual no Linux")
  </linux>
</OSTabs>
...
<OSTabs>
  <windows>
    ![Animação mostrando como listar diferentes pastas e arquivos no Windows](img/20-windows-pastas-cli5.gif "Animação mostrando como listar diferentes pastas e arquivos no Windows")
  </windows>
  <linux>
    ![Animação mostrando como listar diferentes pastas e arquivos no Linux](img/20-linux-pastas-cli5.gif "Animação mostrando como listar diferentes pastas e arquivos no Linux")
  </linux>
</OSTabs>
...
Praticamente todos os comandos contam com uma sessão de ajuda. Você pode acessar essa ajuda com o parâmetro `-?`, `-h`, `--help` *(do inglês help: ajuda, em português)* ou no caso de comandos mais antigos do Windows, com `/?`:
...
Outra dica bacana de saber: muitos comandos mais modernos contam com o que são chamados de parâmetro resumido e parâmetros extensos. Vimos um breve exemplo disso com `-h` e `--help`: eles são a mesma coisa, mas um é resumido (apenas 1 hífen e apenas 1 letra) e o outro é a versão extensa (2 hífens e uma palavra). E **alguns desses parâmetros recebem valores!** Por exemplo:
