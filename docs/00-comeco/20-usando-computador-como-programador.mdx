...
Praticamente todos os comandos contam com uma sessão de ajuda. Você pode acessar essa ajuda com o parâmetro `-h`, `--help` ou no caso de comandos mais antigos do Windows, com `/?`:

```text
PS C:\Users\rafael> ping /?
...
Cada comando tem sua própria lista de parâmetros específicas do comando. No exemplo abaixo, vemos o comando `ls` sem parâmetros, que lista os arquivos e pastas do diretório atual sem nenhuma formatação. Logo em seguida, é passado o parâmetro `-l` que instrui o comando a mostrar a lista detalhada.
